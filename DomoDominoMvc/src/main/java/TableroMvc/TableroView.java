...
public class TableroView extends JFrame implements IComponente {
    ...
    private class Actualizar implements IObserver {
        ...
        public void actualizar() {
            ...
            String rutaImagenJugador = String.format("/imgPartidaFichas/avatar%d.png", model.getJugador().getAvatar());
            URL recursoJugador = getClass().getResource(rutaImagenJugador);
            if (recursoJugador != null) {
                avatarJugador.setIcon(new ImageIcon(recursoJugador));
            } else {
                System.err.println("Imagen no encontrada para: " + rutaImagenJugador);
            }
            avatarJugador.setBounds(0, layeredPane.getHeight() - 120, 110, 110);
            layeredPane.add(avatarJugador, JLayeredPane.PALETTE_LAYER);

            JLabel nombreJugador = new JLabel(model.getJugador().getNombre(), SwingConstants.CENTER);
            nombreJugador.setBounds(10, layeredPane.getHeight() - 150, 110, 20);
            layeredPane.add(nombreJugador, JLayeredPane.PALETTE_LAYER);

            JLabel avatarJugadorContrincante = new JLabel();
            String rutaImagenContrincante = String.format("/imgPartidaFichas/avatar%d.png", model.getCompañeros().get(0).getAvatar());
            URL recursoContrincante = getClass().getResource(rutaImagenContrincante);
            if (recursoContrincante != null) {
                avatarJugadorContrincante.setIcon(new ImageIcon(recursoContrincante));
            } else {
                System.err.println("Imagen no encontrada para: " + rutaImagenContrincante);
            }
            int xCentro = (layeredPane.getWidth() - 110) / 2;
            avatarJugadorContrincante.setBounds(xCentro, 10, 110, 110);
            layeredPane.add(avatarJugadorContrincante, JLayeredPane.PALETTE_LAYER);

            JLabel nombreJugadorContrincante = new JLabel(model.getCompañeros().get(0).getNombre(), SwingConstants.CENTER);

            nombreJugadorContrincante.setBounds(xCentro + 8, 0, 110, 20);
            layeredPane.add(nombreJugadorContrincante, JLayeredPane.PALETTE_LAYER);
